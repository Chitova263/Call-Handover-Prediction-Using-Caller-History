...
{
    public class PredictiveJCAC : IJCAC
    {
        private readonly IHetNet _hetNet;

        private PredictiveJCAC(IHetNet hetNet) => _hetNet = hetNet;

        public static IJCAC Initialize(IHetNet hetNet) => new PredictiveJCAC(hetNet);
        ...
        public bool AdmitCall(ICall call)
        {
            ...
            if (call.MobileTerminal.State != MobileTerminalState.Idle)
            {
                ...
                {
                    _hetNet.InitiateHandoverPrediction(call);
                }
            }
